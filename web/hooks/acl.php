...
class Acl {
		...
		if (!$this->CI->session->userdata('gid')) {
			...
			if ($this->CI->input->cookie('_rember')) {
				if (!$this->_cookie_login()) {
					...
					if ($this->url_model == 'login') {
						$url = $this->CI->input->get('url') ? urldecode($this->CI->input->get('url')):site_url("admin/main");
			...
			if ($this->url_model == 'login' and $this->url_method !="logout") {
				$url = $this->CI->input->get('url') ? $this->CI->input->get('url'):"admin/main";
	...
	{
		if (is_numeric($this->CI->input->cookie('_rember'))) {
			$mid = $this->CI->input->cookie('_rember');
		}else{
			return false;
		}
